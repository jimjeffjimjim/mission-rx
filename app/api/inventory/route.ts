    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        const mapped = data.map((item: any) => ({
          ...
          initialBottlesAvailable: item.initial_bottles_available ?? item.bottles_available,
          initialLooseUnitsAvailable: item.initial_loose_units_available ?? item.loose_units_available,
        ...
        try {
          const seedPayload = fullClinicItems.map(item => ({
            ...
            initial_bottles_available: item.bottlesAvailable,
            initial_loose_units_available: item.looseUnitsAvailable,
...
export async function POST(request: Request) {
  ...
  const payload = {
    ...
    initialBottlesAvailable: Number(data.bottlesAvailable) || 0,
    initialLooseUnitsAvailable: Number(data.looseUnitsAvailable) || 0,
      ...
      const { error: insertError } = await supabase.from('inventory_items').insert([
        {
          ...
          initial_bottles_available: payload.initialBottlesAvailable,
          initial_loose_units_available: payload.initialLooseUnitsAvailable,
